...
import threading
import os
import urllib, urllib3, http.cookiejar, sys
...
def SMS_window(master):
	...
	text = Text(root, bg= 'lemon chiffon', height=9, width=35)
	frame1 = Frame(root)
	frame1.pack()
	...
	l1 = Label(frame1, text="Enter Login ID:")
	...
	l3 = Label(frame2, text="Trying to Login...")
	l4 = Label(frame2, text="Connected successfully")
	l5 = Label(frame3, text="Enter mobile number to send SMS:")
	e3 = Entry(frame3, textvariable = v3)
	#e4 = Entry(frame3, width=30, textvariable = v4)
	text.insert(INSERT, "Message should be of 160 words or less!")
	b2 = Button(bottomFrame, text="Send SMS", command=sendSMS)
	l6 = Label(bottomFrame, text="SMS sent successfully!!")
	b3 = Button(bottomFrame, text="Exit", command=quit)
...
def sendSMS():
		...
		mobile=v3.get()
		while True:
			message_raw=text.get("1.0","end-1c")
...
if len(sys.argv) > 1 and sys.argv[1] == "-cli":
    ...
    menubar.add_command(label="FreeSMS", command=lambda:SMS_window(root))
    ...
    main_body_text.insert(END, "Welcome to the IntraChat!")
    main_body_text.config(state=DISABLED)
